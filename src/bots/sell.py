...
class Sell:
    def __init__(self, sender_address: str, private_key: str, token_to_sell: str,
                 quantity: float, gas_amount: int, gas_price: str, logger: object):

        # add your blockchain connection information
        self.bsc = 'https://bsc-dataseed1.defibit.io/'
        ...
        self.wbnb_address = self.web3.toChecksumAddress('0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c')  # WBN
        self.sender_address = self.web3.toChecksumAddress(sender_address)
        self.token_to_sell_address = self.web3.toChecksumAddress(token_to_sell)
        ...
        self.contract = self.web3.eth.contract(address=self.panRouterContractAddress, abi=panabi)
        ...
        self.sellTokenContract = self.web3.eth.contract(self.token_to_sell_address, abi=sellAbi)

        # get decimals, each need to be handle differently)
        self.decimals = int("1" + str("0" * self.sellTokenContract.functions.decimals().call()))
        print(self.decimals)

        # TODO self.TradingTokenDecimal = decimalData.getTokenDecimal(int(self.decimals))
    ...
    def get_balance(self):
        ...
        humanReadable = self.web3.fromWei(balance, unit='ether')
        level = logging.WARNING
        self.logger.log(level, f"Your BNB balance: " + str(humanReadable))

        # Get Token to sell ballance
        tk_to_sell_balance = self.sellTokenContract.functions.balanceOf(self.sender_address).call()

        readable = tk_to_sell_balance / self.decimals
        # TODO readable = self.web3.fromWei(tk_to_sell_balance, self.TradingTokenDecimal)

        self.logger.log(level, f"Your {self.symbol} Balance:" + str(readable))

        return readable

    def get_token_value_usd(self, tokenAddress):
        ...
        token_value = float(self.balance) * float(price)
        return token_value

    def approve_token(self):
        # Approve Token before Selling
        balance = self.sellTokenContract.functions.balanceOf(self.sender_address).call()

        approve = self.sellTokenContract.functions.approve(self.panRouterContractAddress, balance).buildTransaction({
            'from': self.sender_address,
            'gasPrice': self.web3.toWei(Decimal(self.gas_price), 'gwei'),
        ...
        print("Approved: " + self.web3.toHex(tx_token))
        level = logging.WARNING
        self.logger.log(level, self.symbol + " Approved")
        self.logger.log(level, "Transaction hash: " + self.web3.toHex(tx_token))

    #  Receive an as much BNB as possible for an exact amount of input tokens.
    def sell_token(self):

        tokenValue = float(self.balance)

        tokenValue2 = int(tokenValue * self.quantity) * self.decimals

        # TODO tokenValue2 = self.web3.toWei(int( tokenValue * self.quantity), self.TradingTokenDecimal)

        print(f"Swapping {tokenValue2} {self.symbol} for BNB")

        level = logging.CRITICAL
        self.logger.log(level, f"Swapping {tokenValue} {self.symbol} for BNB")
        self.logger.log(level, "Please wait this may take few seconds...")

        time.sleep(15)

        try:
            pancakeswap2_txn = self.contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(
                tokenValue2, # Payable amount of input tokens
                0,  # The minimum amount tokens to receive aka slippage
                [self.token_to_sell_address, self.wbnb_address],
                self.sender_address,  # Address of recipient
                (int(time.time()) + 1000000)  # deadline
            ...
            print(f"Sold {self.symbol}: " + self.web3.toHex(tx_token))
            level = logging.WARNING
            self.logger.log(level, "Swap was successful")
            self.logger.log(level, f"Sold {tokenValue} {self.symbol} for BNB")
            self.logger.log(level, "Transaction Hash: " + self.web3.toHex(tx_token))

...
def buy(tokenToBuy):
    ...
    try:
        pancakeswap2_txn = contractbuy.functions.swapExactETHForTokens(
            ...
            'from': sender_address,
            'value': web3.toWei(0.003, 'ether'),  # This is the Token(BNB) amount you want to Swap from
        ...
        print("Snipe was succesfull, bought: " + tokenToBuy)
        print("Transaction adress: " + web3.toHex(tx_token))
...
def handle_event(event):
    ...
    if(token0.upper().strip('"') == wbnb2):
        tokenToBuy = web3.toChecksumAddress(token1.upper().strip('"'))
        print("pair detected")
        buy(tokenToBuy)
    elif(token1.upper().strip('"') == wbnb2):
        tokenToBuy = web3.toChecksumAddress(token0.upper().strip('"'))
        print("pair detected")
        buy(tokenToBuy)
    else:
        print("next pair")
...
def main():
    ...
    #finally:
        ...
        #loop.close()
    # onyx 0xbc45EDd4b1D3bC9AA665232055cbdDAE64Ef503e

    tokenToBuy = web3.toChecksumAddress("0xbc45EDd4b1D3bC9AA665232055cbdDAE64Ef503e")
    helper = False
